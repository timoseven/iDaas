...
> 一次性配置。完成后将 **IdP ARN** 回填到 iDaas 的 `SAML_IDP_ARN`，将 **角色 ARN** 在 iDaas 后台登记为 RAM 角色。
...
- [ ] 将该 ARN 回填到 iDaas 的环境变量 `SAML_IDP_ARN` 并重启 iDaas
...
- [ ] 若失败：检查 `SAML_IDP_ARN` 是否已填、角色是否启用、用户是否绑定、iDaas 系统时间是否准确（Assertion 有 ±1min 时钟容差与 5min 有效期）

## 常见问题
- **签名校验失败**：metadata 中的证书与签名所用私钥不匹配，或 metadata 未更新到最新证书（重新上传 metadata）。
- **「无权访问」/「Role not found」**：`SAML_IDP_ARN` 与角色 ARN 主账号 ID 不一致，或角色未信任该 IdP。
- **角色控制台报「未配置 SAML_IDP_ARN」**：阿里云侧 IdP 尚未创建，或 ARN 未回填到环境变量。
